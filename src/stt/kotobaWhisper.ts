...
function resolveServerCommandSpec(): CommandSpec {
  ...
  if (!fs.existsSync(scriptPath)) {
    throw new Error(`kotoba-whisperサーバーが見つかりません: ${scriptPath}`);
  }
...
function isPersistentEnabled(): boolean {
  const raw = process.env.KOTOBA_WHISPER_PERSISTENT?.trim();
  if (!raw) {
    return true;
  }
...
function getWorkerCount(): number {
  const raw = Number.parseInt(process.env.KOTOBA_WHISPER_WORKERS ?? "", 10);
...
function logDebug(message: string): void {
  if (process.env.KOTOBA_WHISPER_DEBUG === "1") {
    console.log(message);
  }
}
    ...
    const startPromise = new Promise<void>((resolve, reject) => {
      ...
      proc.on("exit", (code, signal) => {
        const error = new Error(
          `kotoba-whisperプロセスが終了しました (code=${code ?? "unknown"} signal=${
            signal ?? "unknown"
          })`
        );
        this.handleExit(error);
